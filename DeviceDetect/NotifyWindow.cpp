...
static CNotifyWindow* g_NotifyWindow = NULL;

CNotifyWindow::CNotifyWindow(IDeviceChanged& deviceChanged)
	: hWnd_(NULL)
	, destroy_(false)
	, thread_(CNotifyWindow::ThreadProc)
	, started_()
...
CNotifyWindow::~CNotifyWindow(void)
{
	destroy_ = true;
...
void CNotifyWindow::NotifyRegisterClass(HINSTANCE hInstance)
{
	WNDCLASSEX wcex;
	...
	wcex.lpfnWndProc	= CNotifyWindow::WndProc;
	wcex.cbClsExtra		= 0;
	wcex.cbWndExtra		= 0;
...
bool CNotifyWindow::InitInstance(HINSTANCE hInstance)
{
...
LRESULT CALLBACK CNotifyWindow::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message)
	{
	case WM_PAINT:
...
HWND CNotifyWindow::GetHWND(void) const
{
	return hWnd_;
}

void CNotifyWindow::ThreadProc(void* context)
{
	CNotifyWindow* This = static_cast<CNotifyWindow*>(context);
