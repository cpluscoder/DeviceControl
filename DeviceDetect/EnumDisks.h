...
namespace Disk
{
	namespace Types
	{
		...
		struct RemovableDeviceInfo
		{
			void Clear(void)
			{
				wsDevInterfaceVolume.clear();
				wsDeviceDesc.clear();
				wsEnumeratorName.clear();
				wsCompatibleIDs.clear();
				wsHardwareID.clear();
				wsMFG.clear();
				wsFriendlyName.clear();
				wsPath.clear();
				deviceType = rdtUnknown;
			}
	...
	namespace Functions
	{
		bool SearchRemovalDisks(Types::RemovableDeviceInfo_vt &vctDevice);
	}
}
